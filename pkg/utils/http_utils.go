...
func GetFileRequest(url string) (*http.Response, error) {
	...
	if err != nil {
		return nil, pkg.WrapError(err, pkg.ErrorCodeUnknown, "Failed to fetch file")
	}

	// Check if the response status is 200 OK
	if response.StatusCode != http.StatusOK {
		// return nil, pkg.NewError(pkg.ErrorCodeUnknown, fmt.Sprintf("Failed to fetch image, status code: %d", response.StatusCode))
